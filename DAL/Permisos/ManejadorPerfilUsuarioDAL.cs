    ...
    public class ManejadorPerfilUsuarioDAL
    {
        ...
        public void AsignarOperacionesaUsuario(Usuario usuBe, List<Operacion> listaoperacioneUsuario)
        {
            ...
            string primersql = " delete usuariooperacion where usuarioid = (select Usuarioid " +
                                " from usuario where Usuario like '" + usuBe._Usuario + "'); ";
            ...
            string sql;
            foreach (BE.Seguridad.Operacion item in listaoperacioneUsuario)
            {
                sql = " INSERT INTO usuariooperacion (UsuarioID,OperacionID,Habilitado)" +
                      ...
                      " and usu.Usuario like '" + usuBe._Usuario.ToString() + "';";
        ...
        public PerfilUsuario AsignarUsuarioaPerfil(PerfilUsuario pUbe, Usuario usuBe)
        {
            string sql = "  Delete usuariofamilia where UsuarioID = ( " +
                       "  select UsuarioID from usuario where usuario like '%" + usuBe._Usuario + "%');" +
                       ...
                       "  where PerfilUsuarioID in (select PerfilUsuarioID from PerfilUsuario where NombrePerfil like '" + pUbe.NombrePerfil + "') " +
                       "  and usuarioID in (select UsuarioID from usuario where usuario like '%" + usuBe._Usuario + "%') ;" +
                       "Delete usuariooperacion where UsuarioID = ( " +
                       "  select UsuarioID from usuario where usuario like '" + usuBe._Usuario + "');" +
                       " INSERT INTO usuariooperacion (UsuarioID, OperacionID, Habilitado, DVH) " +
                       " (select U.UsuarioID, PO.OperacionID, 'S', '0' from PerfilOperacion PO " +
                       "    INNER JOIN PerfilUsuario PU ON PU.PerfilUsuarioID = PO.PerfilUsuarioID " +
                       " INNER JOIN UsuarioFamilia UF ON UF.PerfilID = PO.PerfilUsuarioID " +
                       " INNER JOIN Usuario U ON U.UsuarioID = UF.UsuarioID " +
                       " WHERE PU.NombrePerfil = '" + pUbe.NombrePerfil+"' "+  
                       " AND U.Usuario = '"+ usuBe._Usuario + "') ";
        ...
        public void AsignarOperacionesalPerfil(PerfilUsuario perfilBE, List<Operacion> listaoperacionesperfil)
        {
            try
            {
                string primersql = "delete perfiloperacion where PerfilUsuarioID = " + perfilBE.PerfilUsuarioID;
                con.Ejecutar(primersql);
                con.Desconectar();
                ...
                foreach (BE.Seguridad.Operacion item in listaoperacionesperfil)
                {
                    ...
                    sql = "INSERT INTO perfiloperacion (OperacionID,PerfilUsuarioID)" +
                          " SELECT OperacionID, " + perfilBE.PerfilUsuarioID + " " +
                          " FROM OPERACION op" +
                          " WHERE op.Descripcion like '" + item.NombreOperacion.ToString() + "';";
                    con.Ejecutar(sql);
